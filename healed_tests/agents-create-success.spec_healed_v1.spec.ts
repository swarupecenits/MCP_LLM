...
test("Navigate to portal, go to Agents, refresh, create new agent and check success", async () => {
  ...
  // Updated selector to use a more robust approach
  await page.locator('[data-testid="new-agent-button"]').click();

  // Step 5: Check for the 'Success' message
  // Updated selector for the success alert to ensure stability
  const successLocator = page.locator('[role="alert"][data-testid="success-alert"]');
