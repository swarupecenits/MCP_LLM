...
test("Azure AI Foundry Language Playground navigation", async () => {
  ...
  await page.getByRole("link", { name: "Playgrounds" }).waitFor({ state: "visible", timeout: 20000 });

  // Step 2: Go to Playgrounds
  await page.getByRole("link", { name: "Playgrounds" }).click();
  await page.getByRole("button", { name: "Try the Language playground" }).waitFor({ state: "visible", timeout: 20000 });

  // Step 3: Go to Language Playground
  await page.getByRole("button", { name: "Try the Language playground" }).click();
  await page.getByRole("button", { name: "All", exact: true }).waitFor({ state: "visible", timeout: 20000 });
