...
test("Azure AI Foundry Language Playground navigation", async () => {
  ...
  await page.getByText("Playgrounds").first().waitFor({ state: "visible", timeout: 30000 }); // Ensure "Playgrounds" is visible

  // Step 2: Go to Playgrounds
  await page.getByRole("link", { name: "Playgrounds" }).click();
  await page.waitForTimeout(1000); // Added delay for stability
