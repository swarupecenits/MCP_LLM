...
test("Azure AI Foundry Language Playground navigation", async () => {
  ...
  // Replaced flaky selector with a more robust one
  await page.getByRole("link", { name: "Playgrounds" }).waitFor({ state: "visible", timeout: 30000 });
  ...
  await page.waitForTimeout(1000); // Added retry logic for stability
  ...
  await page.getByText("Extract key phrases").waitFor({ state: "visible", timeout: 30000 });
