...
test("Azure AI Foundry Language Playground navigation", async () => {
  ...
  try {
    ...
    const panelButtons = [
      { name: "All", locator: page.getByRole("button", { name: "All", exact: true }) },
      { name: "Extract Information", locator: page.getByRole("button", { name: "Extract Information" }) },
      { name: "Summarize Information", locator: page.getByRole("button", { name: "Summarize Information" }) },
      { name: "Classify Text", locator: page.getByRole("button", { name: "Classify Text" }) },
      { name: "Fine-tune models", locator: page.getByRole("button", { name: "Fine-tune models" }) },
    ];

    for (const button of panelButtons) {
      await button.locator.waitFor({ state: "visible", timeout: 15000 });
      await button.locator.click();
      await expect(button.locator).toBeVisible({ timeout: 15000 });
    }

    // Step 5: Verify cards in the panel
    const extractKeyPhrasesCard = page.getByText("Extract key phrases");
    await extractKeyPhrasesCard.waitFor({ state: "visible", timeout: 15000 });
    ...
    await allButton.waitFor({ state: "visible", timeout: 15000 });
    await allButton.click();

    const analyzeSentimentCard = page.getByText("Analyze sentiment");
    await analyzeSentimentCard.waitFor({ state: "visible", timeout: 15000 });
